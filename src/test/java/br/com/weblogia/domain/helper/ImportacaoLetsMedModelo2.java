...
public class ImportacaoLetsMedModelo2 {
	...
	public static void main(String[] args) throws IOException, ParseException {
		File initialFile = new File("C:\\Borius\\amazon\\importacao2.xls");
		...
		for (Row row : sheet){
			...
			for (Cell cell : row){
				...
				if (cell.getColumnIndex() == 0){
					...
					if (row.getCell(6) != null){
						...
						try{
							...
							shipDate = row.getCell(6).getDateCellValue();
						}catch (IllegalStateException e){
							shipDate = new SimpleDateFormat("dd/MM/yyyy").parse(row.getCell(6).getStringCellValue());
						}
				...
				if (cell.getColumnIndex() == 11){
					...
					oi.setCommision(row.getCell(15).getNumericCellValue());
